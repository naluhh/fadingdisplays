...
app.get('/library/:id', function(req, res) {
    ...
    let request = 'SELECT ID id, FILENAME filename FROM LIBRARY WHERE ID = ?';
    try {
	db.get(request, [req.params.id], (err, row) => {
		...
		if (found == false) {
		    ...
		    const file = `${__dirname}/library/` + row.filename;
		    res.download(file);
		}
	    }
...
app.delete('/library/:id', function(req, res) {
	...
	db.get(request, [id], (err, row) => {
		...
		if (found == false) {
		    ...
		    let test_current_request = 'SELECT IMAGE_ID id FROM CURRENT_IMAGE WHERE IMAGE_ID = ?';
		    db.get(test_current_request, [id], (err, row) => {
			if (row) {
			    res.status(400).send('image currently selected');
			} else {
			    let delete_request = 'DELETE FROM LIBRARY WHERE ID = ?'

			    db.run(delete_request, [id], function(err) {
				if (err) {
				    res.status(500).send('INTERNAL DB ERROR->' + err);
				}

				var path = "library/" + row.filename;
				fs.unlink(path, (err) => {
				    if (err) {
					console.error(err)
				    }
